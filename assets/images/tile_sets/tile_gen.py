...
def treat_image(path, bg, frames):
    im = Image.open(path)
    background = None
    if bg: background = Image.open(bg)
...
def save_result(frames, filename):
    ...
    rows = 8
    cols  = 6
    result_final = Image.new('RGBA', (cols * imgwidth, imgheight * rows))

    for index, frame in enumerate(frames):
        for i, img in enumerate(frame):
            result_final.paste(img, (i // rows * imgwidth, i % rows * imgheight), img)
